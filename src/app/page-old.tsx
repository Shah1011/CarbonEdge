...
export default function Home(): JSX.Element {
    ...
    const [glassSize, setGlassSize] = useState({ width: 480, height: 520 });
    useEffect(() => {
      setMounted(true);
      function updateSize() {
  ...
  const [forecastRegionsLoading, setForecastRegionsLoading] = useState<boolean>(false);
  ...
  // Fetch TFT carbon forecast for a given provider + region
  const fetchForecast = async (provider: string, region: string): Promise<void> => {
    console.log('[fetchForecast] Requesting:', { provider, region });
    ...
    try {
      ...
      if (res.ok) {
        ...
        console.log('[fetchForecast] Response:', json);
        if (!json.error) {
          setForecastData(json as ForecastData);
        } else {
          console.warn('[fetchForecast] Error in response:', json.error);
          setForecastData(null);
        }
      }
    } catch (err) {
      console.error('[fetchForecast] Request failed:', err);
      setForecastData(null);
    } finally {
  ...
  // Fetch all forecast regions (for Forecast mode)
  const fetchAllForecasts = async (): Promise<void> => {
    ...
    try {
      const selectedProviders = (Object.keys(form.providers) as Array<keyof ProviderFlags>)
        .filter((p) => form.providers[p]);
      ...
      if (res.ok) {
        const json = await res.json();
        if (json.regions) {
          setForecastRegions(json.regions);
        }
      }
    } catch (err) {
      console.error("[fetchAllForecasts] Failed:", err);
    } finally {
      ...
      <div className="flex w-screen h-screen">
        <div className="flex-1 flex flex-col justify-center items-center p-2 overflow-y-auto ml-14 h-full">
          {mounted && accordionOpen && (
            ...
            {/* Mode Toggle – outside the card */}
            <div className="flex items-center justify-center bg-white/10 rounded-lg p-1 gap-1 shadow w-full">
              <button
                type="button"
                onClick={() => setAppMode("decision")}
                className={`flex-1 px-3 py-1 rounded-md text-sm font-semibold transition-all duration-200 ${
                  appMode === "decision"
            ...
            <GlassSurface
              ...
              className="p-6 shadow-2xl mt-0 w-full"
            >
                <div
                ...
                {appMode === "decision" ? (
                  <>
                <h2 className="text-xl font-bold mb-3 text-white">Select Resource</h2>
                <div className="mb-3">
                ...
                <button
                  type="submit"
                  className="w-full mt-4 px-6 py-2 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 transition flex items-center justify-center"
                  disabled={loading}
                >
                  {loading && (
                  ...
                  /* ── Forecast Mode ── */
                  <>
                    <h2 className="text-xl font-bold mb-4 text-white">Carbon Forecast Explorer</h2>
                    <p className="text-white/60 text-xs mb-4">
                      View 7-day carbon intensity forecasts for all regions with available data. 
                      Regions are ranked by eco-efficiency — lowest forecast carbon first.
                    </p>
                    <div className="mb-4">
                      <span className="block mb-2 font-semibold text-white text-sm">Filter by Provider</span>
                      <div className="flex gap-4 mt-2">
                        <label className="flex items-center gap-2">
                          <input type="checkbox" name="aws" checked={form.providers.aws} onChange={handleChange} className="accent-emerald-600" />
                          <span className="text-white">AWS</span>
                        </label>
                        <label className="flex items-center gap-2">
                          <input type="checkbox" name="azure" checked={form.providers.azure} onChange={handleChange} className="accent-emerald-600" />
                          <span className="text-white">Azure</span>
                        </label>
                        <label className="flex items-center gap-2">
                          <input type="checkbox" name="gcp" checked={form.providers.gcp} onChange={handleChange} className="accent-emerald-600" />
                          <span className="text-white">GCP</span>
                        </label>
                      </div>
                    </div>
                    <button
                      ...
                      className="w-full mt-2 px-6 py-3 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 transition flex items-center justify-center"
                      disabled={forecastRegionsLoading}
                    >
                      {forecastRegionsLoading && (
                        <span className="w-5 h-5 border-4 border-t-emerald-300 border-gray-300 rounded-full animate-spin mr-2 inline-block align-middle"></span>
                      )}
                      {forecastRegionsLoading ? "Loading Forecasts..." : "Get Eco-Optimized Forecasts"}
                    </button>
                  </>
                )}
                </form>
          ...
          {!accordionOpen && appMode === "forecast" && forecastRegions.length > 0 && (
            <GlassSurface
              borderRadius={24}
              width={glassSize.width}
              ...
              <div className="w-full">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-bold text-white">
                    Eco-Optimized Regions
                    <span className="ml-2 text-xs font-normal text-white/60">
                      ({forecastRegions.length} regions with forecast data)
                    </span>
                  </h3>
                </div>
                <div className="max-h-[55vh] overflow-y-auto scrollbar-none flex flex-col gap-3" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                  {forecastRegions.map((fr, idx) => {
                    const cardKey = `fc-${fr.group_id}`;
                    const trendColor = fr.summary.trend === "decreasing" ? "text-green-600 bg-green-50" : fr.summary.trend === "increasing" ? "text-red-500 bg-red-50" : "text-yellow-600 bg-yellow-50";
                    const trendIcon = fr.summary.trend === "decreasing" ? "↓" : fr.summary.trend === "increasing" ? "↑" : "→";
                    const providerLabel = fr.provider.toLowerCase() === "aws" ? "Amazon Web Services" : fr.provider.toLowerCase() === "azure" ? "Microsoft Azure" : fr.provider.toLowerCase() === "gcp" ? "Google Cloud" : fr.provider;
                    const handleForecastCardClick = () => {
                      ...
                      fetchForecast(fr.provider, fr.region);
                    };
                    return (
                      <div
                        key={cardKey}
                        className="bg-white rounded-xl shadow-md p-4 border border-gray-200 cursor-pointer w-full hover:shadow-lg transition-shadow"
                        onClick={handleForecastCardClick}
                      >
                        <div className="flex items-center justify-between mb-2">
                          <div className="font-semibold flex items-center gap-3">
                            <img
                              src={`/${fr.provider.toLowerCase()}.png`}
                              alt={fr.provider.toUpperCase() + ' logo'}
                              style={{ height: '1.75em', width: '1.75em', objectFit: 'contain' }}
                              className="inline"
                              onError={e => { (e.target as HTMLImageElement).style.display = 'none'; }}
                            />
                            <span className="flex flex-col">
                              <span className="text-sm">{fr.region}</span>
                        ...
                        <div className="flex items-center gap-4 text-sm">
                          <div className="text-gray-700">
                            <span className="font-semibold">Forecast Avg:</span>{" "}
                            {fr.summary.forecast_avg_gCO2} {fr.summary.unit}
                          </div>
                          <div className="text-gray-700">
                            <span className="font-semibold">Recent Avg:</span>{" "}
                            {fr.summary.recent_avg_gCO2} {fr.summary.unit}
                          </div>
                        </div>
                        <div className="mt-1.5 flex items-center gap-2">
                          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${trendColor}`}>
                            {trendIcon} {fr.summary.trend}
                          </span>
                          <span className="text-xs text-gray-400">
                            {fr.summary.prediction_days}-day forecast
                          </span>
                        </div>
                      </div>
                    );
                ...
                <button
                  className="mt-4 w-full px-6 py-3 bg-emerald-600 text-white rounded-lg font-semibold hover:bg-emerald-700 transition"
                  onClick={() => {
          ...
          {/* Results Grid */}
          {!accordionOpen && appMode === "decision" && results && (
              <GlassSurface
                borderRadius={24}
                width={glassSize.width}
                height={glassSize.height * 1.1}
                          ...
                          <div className="flex flex-col gap-4 items-center w-full overflow-y-auto max-h-full" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                            ...
                            const handleCardClick = () => {
                              ...
                              if (coords) {
                                ...
                                fetchForecast(provider, instance.region);
                              } else {
                                ...
                                {instance.carbon_forecast && (
                                  <div className="mt-1">
                                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${
                                      ...
                                      {instance.carbon_forecast.trend === 'decreasing' ? '↓' : instance.carbon_forecast.trend === 'increasing' ? '↑' : '→'}
                                      {' '}Forecast: {instance.carbon_forecast.forecast_avg_gCO2} {instance.carbon_forecast.unit} (7d avg)
                                    </span>
                                  </div>
                                )}
        ...
        <div className="flex-1 flex flex-col justify-center items-center bg-transparent">
          ...
          {/* TFT Carbon Forecast Chart */}
          {forecastLoading && (
            <div className="mt-2 text-xs text-gray-400 flex items-center gap-2">
              <span className="w-4 h-4 border-2 border-t-blue-500 border-gray-300 rounded-full animate-spin inline-block"></span>
              Loading forecast…
            </div>
          )}
